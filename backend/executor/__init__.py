...
async def execute_login(username: str, password: str, iam_url: str) -> Job:
    ...
    # Write password to a temp file (beamctl -p expects a file path)
    pass_file = Path(BIN_DIR) / ".beamctl_pass"
    pass_file.write_text(password)

    full_command = [cli_path, "login", "-u", username, "-p", str(pass_file), "-t", iam_url]
...
async def execute_bamctl_login(username: str, password: str, iam_url: str) -> Job:
    """Execute bamctl login command, auto-accepting certificate trust prompt."""
    cli_path = get_cli_path("bamctl")

    # Write password to a temp file (bamctl -p expects a file path)
    pass_file = Path(BIN_DIR) / ".bamctl_pass"
    pass_file.write_text(password)
